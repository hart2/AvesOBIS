...
# Using lat/long see what environmental data is causing shift
#   Use vegan package, classical RDA w/ AIC
# Need environmental data for the specific points in time (Copernicus data 
# averaged seasonally outside of R using SNAP)
...
grisEV_eastfall90s <- as.data.frame(grisEV_eastfall90s, xy = TRUE) # Raster into data frame
grisEV_eastfall90s <- grisEV_eastfall90s %>%                       # Add column with Fall 90s
  add_column(Season = "Fall",
             Decade = "90s",
             .after = "y")
colnames(grisEV_eastfall90s) <- c("x","y","Season","Decade","EV")  # Change last column name to "EV"

grisEV_eastfall00s <- raster("Extract_EVgriseus_eastfall00s.tif")
grisEV_eastfall00s <- as.data.frame(grisEV_eastfall00s, xy = TRUE) # Raster into data frame
grisEV_eastfall00s <- grisEV_eastfall00s %>%                       # Add column with Fall 00s
  add_column(Season = "Fall",
             Decade = "00s",
             .after = "y")
colnames(grisEV_eastfall00s) <- c("x","y","Season","Decade","EV")  # Change last column to "EV"

grisEV_eastfall10s <- raster("Extract_EVgriseus_eastfall10s.tif")
grisEV_eastfall10s <- as.data.frame(grisEV_eastfall10s, xy = TRUE) # Raster into data frame
grisEV_eastfall10s <- grisEV_eastfall10s %>%                       # Add column with Fall 10s
  add_column(Season = "Fall",
             Decade = "10s",
             .after = "y")
colnames(grisEV_eastfall10s) <- c("x","y","Season","Decade","EV")  # Change last column to "EV"
...
grisEV_westfall90s <- as.data.frame(grisEV_westfall90s, xy = TRUE)      # Raster into data frame 
grisEV_westfall90s <- grisEV_westfall90s %>%                            # Add column with Fall 90s
  add_column(Season = "Fall",
             Decade = "90s",
             .after = "y")
colnames(grisEV_westfall90s) <- c("x","y","Season","Decade","EV")       # Change last column name to "EV"

grisEV_westwinter00s <- raster("Extract_EVgriseus_westwinter00s.tif")
grisEV_westwinter00s <- as.data.frame(grisEV_westwinter00s, xy = TRUE)  # Raster into data frame
grisEV_westwinter00s <- grisEV_westwinter00s %>%                        # Add column with Winter 00s
  add_column(Season = "Winter",
             Decade = "00s",
             .after = "y")
colnames(grisEV_westwinter00s) <- c("x","y","Season","Decade","EV")     # Change last column name to "EV"

grisEV_westspring90s <- raster("Extract_EVgriseus_westspring90s.tif")
grisEV_westspring90s <- as.data.frame(grisEV_westspring90s, xy = TRUE)  # Raster into data frame
grisEV_westspring90s <- grisEV_westspring90s %>%                        # Add column with Spring 90s
  add_column(Season = "Spring",
             Decade = "90s",
             .after = "y")
colnames(grisEV_westspring90s) <- c("x","y","Season","Decade","EV")     # Change last column name to "EV"
...
nigEV_fall90s <- as.data.frame(nigEV_fall90s, xy = TRUE)       # Raster into data frame
nigEV_fall90s <- nigEV_fall90s %>%                             # Add column with Fall 90s
  add_column(Season = "Fall",
             Decade = "90s",
             .after = "y")
colnames(nigEV_fall90s) <- c("x","y","Season","Decade","EV")   # Change last column name to "EV"

nigEV_fall10s <- raster("Extract_EVnigripes_fall10s.tif")
nigEV_fall10s <- as.data.frame(nigEV_fall10s, xy = TRUE)       # Raster into data frame
nigEV_fall10s <- nigEV_fall10s %>%                             # Add column with Fall 10s
  add_column(Season = "Fall",
             Decade = "10s",
             .after = "y")
colnames(nigEV_fall10s) <- c("x","y","Season","Decade","EV")  # Change last column name to "EV"

nigEV_spring90s <- raster("Extract_EVnigripes_spring90s.tif")
nigEV_spring90s <- as.data.frame(nigEV_spring90s, xy = TRUE)   # Raster into data frame
nigEV_spring90s <- nigEV_spring90s %>%                         # Add column with Spring 90s
  add_column(Season = "Spring",
             Decade = "90s",
             .after = "y")
colnames(nigEV_spring90s) <- c("x","y","Season","Decade","EV") # Change last column name to "EV"
...
nigML_fall90s <- as.data.frame(nigML_fall90s, xy = TRUE)      # Raster as data frame
nigML_fall90s <- nigML_fall90s %>%
  filter(Extract_MLnigripes_fall90s >-20)                     # Mixed layer values =< -20 are not real, filter out
nigML_fall90s <- nigML_fall90s %>%                            # Add column with Fall 90s
  add_column(Season = "Fall",
             Decade = "90s",
             .after = "y")
colnames(nigML_fall90s) <- c("x","y","Season","Decade","ML")  # Change last column to "ML"

# Finding the average, maximum, and minimum mixed layer depth
...
nigML_fall10s <- as.data.frame(nigML_fall10s, xy = TRUE)      # Raster as data frame
nigML_fall10s <- nigML_fall10s %>%                            # Mixed layer values =< -20 are not real, filter out
  filter(Extract_MLnigripes_fall10s >-20)
nigML_fall10s <- nigML_fall10s %>%                            # Add column with Fall 90s
  add_column(Season = "Fall",
             Decade = "10s",
             .after = "y")
colnames(nigML_fall10s) <- c("x","y","Season","Decade","ML")  # Change last column to "ML"

nigML_spring90s <- raster("Extract_MLnigripes_spring90s.tif")  # Raster as data frame
nigML_spring90s <- as.data.frame(nigML_spring90s, xy = TRUE)   # Mixed layer values =< -20 are not real, filter out
nigML_spring90s <- nigML_spring90s %>%
  filter(Extract_MLnigripes_spring90s >-20)                    # Add column with Fall 90s
nigML_spring90s <- nigML_spring90s %>%                            
  add_column(Season = "Spring",
             Decade = "90s",
             .after = "y")
colnames(nigML_spring90s) <- c("x","y","Season","Decade","ML") # Change last column to "ML"
...
nig <- nig %>%
  filter(Sa > 0)                    # Salinity values =< 0 aren't real, remove

# Remove unncessary variables
rm(nigEV_fall10s,nigEV_fall90s,nigEV_spring90s,nigEV_summer00s,nigEV_summer10s,
   nigEV_summer90s,nigML_fall10s,nigML_fall90s,nigML_spring90s,nigML_summer00s,
   nigML_summer10s,nigML_summer90s,nigNV_fall10s,nigNV_fall90s,nigNV_spring90s,
   nigNV_summer00s,nigNV_summer10s,nigNV_summer90s,nigSa_fall10s,nigSa_fall90s,
   nigSa_spring90s,nigSa_summer00s,nigSa_summer10s,nigSa_summer90s,nigSSH_fall10s,
   nigSSH_fall90s,nigSSH_spring90s,nigSSH_summer10s,nigSSH_summer00s,nigSSH_summer90s,
   nigSST_fall10s,nigSST_fall90s,nigSST_spring90s,nigSST_summer00s,nigSST_summer10s,
   nigSST_summer90s,nigEV,nigML,nigNV,nigSa,nigSSH,nigSST)

nigLatLon <- nig[,1:2]              # Obtain latitude and longitudes for all nigripes' occurrence records
# nigLatLon <- latlong2grid(nigLatLon) --> muddies interpretation, possibly gratuitous?
nigEnv    <- nig[,5:ncol(nig)]      # Obtain environmental values for environmental parameters
...
# between the bird population and the set of associated environmental variables.
...
datNig <- cbind(nig, scores_Nig$sites)
datNig$Decade <- factor(datNig$Decade, level = c("90s","00s","10s"))
datNig$Season <- factor(datNig$Season, level = c("Winter","Spring","Summer","Fall"))

ggplot()+
  geom_point(data = datNig, aes(x = RDA1,
                             y = RDA2,
                             color = Decade,
                             fill  = Decade,
                             shape = Season)) +
  ...
  geom_text(data = as.data.frame(scores(rdaNig)$species),
            aes(x = RDA1/optNig_sum$cont$importance[1,1], 
                ...
                label = c("Long","Lat")))+
  stat_ellipse(data = datNig, aes(x = RDA1,
                               y = RDA2,
                               color = Decade),
               size = 1,
               level = 0.90,
...
rm(imEV_fall00s,imEV_fall10s,imEV_fall90s,imEV_spring00s,imEV_spring10s,imEV_spring90s,
   imEV_summer00s,imEV_summer10s,imEV_winter00s,imEV_winter10s,imEV_winter90s,
   imML_fall00s,imML_fall10s,imML_fall90s,imML_spring00s,imML_spring10s,imML_spring90s,
   imML_summer00s,imML_summer10s,imML_winter00s,imML_winter10s,imML_winter90s,
   imNV_fall00s,imNV_fall10s,imNV_fall90s,imNV_spring00s,imNV_spring10s,imNV_spring90s,
   imNV_summer00s,imNV_summer10s,imNV_winter00s,imNV_winter10s,imNV_winter90s,
   imSa_fall00s,imSa_fall10s,imSa_fall90s,imSa_spring00s,imSa_spring10s,imSa_spring90s,
   imSa_summer00s,imSa_summer10s,imSa_winter00s,imSa_winter10s,imSa_winter90s,
   imSSH_fall00s,imSSH_fall10s,imSSH_fall90s,imSSH_spring00s,imSSH_spring10s,imSSH_spring90s,
   imSSH_summer00s,imSSH_summer10s,imSSH_winter00s,imSSH_winter10s,imSSH_winter90s,
   imSST_fall00s,imSST_fall10s,imSST_fall90s,imSST_spring00s,imSST_spring10s,imSST_spring90s,
   imSST_summer00s,imSST_summer10s,imSST_winter00s,imSST_winter10s,imSST_winter90s,
   immerEV,immerML,immerNV,immerSa,immerSSH,immerSST)
...
datIm <- cbind(immer, scores_Immer$sites)
datIm$Decade <- factor(datIm$Decade, level = c("90s","00s","10s"))
datIm$Season <- factor(datIm$Season, level = c("Winter","Spring","Summer","Fall"))

ggplot()+
  geom_point(data = datIm, aes(x = RDA1,
                             y = RDA2,
                             color = Decade,
                             fill  = Decade,
                             shape = Season)) +
  ...
  geom_text(data = as.data.frame(scores(rdaImmer)$species),
            aes(x = RDA1/optImmer_sum$cont$importance[1,1], 
                ...
                label = c("Long","Lat")))+
  stat_ellipse(data = datIm, aes(x = RDA1,
                               y = RDA2,
                               color = Decade),
               size = 1,
               level = 0.90,
...
rm(grisEV_westfall90s,grisEV_westspring90s,grisEV_westsummer00s,grisEV_westsummer10s,
   grisEV_westsummer90s,grisEV_westwinter00s,grisML_westfall90s,grisML_westspring90s,
   grisML_westsummer00s,grisML_westsummer10s,grisML_westsummer90s,grisML_westwinter00s,
   grisNV_westfall90s,grisNV_westspring90s,grisNV_westsummer00s,grisNV_westsummer10s,
   grisNV_westsummer90s,grisNV_westwinter00s,grisSa_westfall90s,grisSa_westspring90s,
   grisSa_westsummer00s,grisSa_westsummer10s,grisSa_westsummer90s,grisSa_westwinter00s,
   grisSSH_westfall90s,grisSSH_westspring90s,grisSSH_westsummer00s,grisSSH_westsummer10s,
   grisSSH_westsummer90s,grisSSH_westwinter00s,grisSST_westfall90s,grisSST_westspring90s,
   grisSST_westsummer00s,grisSST_westsummer10s,grisSST_westsummer90s,grisSST_westwinter00s,
   grisWestEV,grisWestML,grisWestNV,grisWestSa,grisWestSSH,grisWestSST)
...
datGrisWest <- cbind(grisWest, scores_GrisWest$sites)
datGrisWest$Decade <- factor(datGrisWest$Decade, level = c("90s","00s","10s"))
datGrisWest$Season <- factor(datGrisWest$Season, level = c("Winter","Spring","Summer","Fall"))

ggplot()+
  geom_point(data = datGrisWest, aes(x = RDA1,
                             y = RDA2,
                             color = Decade,
                             fill  = Decade,
                             shape = Season)) +
  ...
  geom_text(data = as.data.frame(scores(rdaGrisWest)$species),
            aes(x = RDA1/optGrisWest_sum$cont$importance[1,1], 
                ...
                label = c("Long","Lat")))+
  stat_ellipse(data = datGrisWest, aes(x = RDA1,
                               y = RDA2,
                               color = Decade),
               size = 1,
               level = 0.90,
...
rm(grisEV_eastfall00s,grisEV_eastfall10s,grisEV_eastfall90s,grisEV_eastspring00s,
   grisEV_eastspring10s,grisEV_eastspring90s,grisEV_eastsummer00s,grisEV_eastsummer10s,
   grisEV_eastsummer90s,grisEV_eastwinter10s,grisML_eastfall00s,grisML_eastfall10s,
   grisML_eastfall90s,grisML_eastspring00s,grisML_eastspring10s,grisML_eastspring90s,
   grisML_eastsummer00s,grisML_eastsummer10s,grisML_eastsummer90s,grisML_eastwinter10s,
   grisNV_eastfall00s,grisNV_eastfall10s,grisNV_eastfall90s,grisNV_eastspring00s,
   grisNV_eastspring90s,grisNV_eastspring10s,grisNV_eastsummer00s,grisNV_eastsummer10s,
   grisNV_eastsummer90s,grisNV_eastwinter10s,grisSa_eastfall00s,grisSa_eastfall10s,
   grisSa_eastfall90s,grisSa_eastspring00s,grisSa_eastspring10s,grisSa_eastspring90s,
   grisSa_eastsummer00s,grisSa_eastsummer10s,grisSa_eastsummer90s,grisSa_eastwinter10s,
   grisSSH_eastfall00s,grisSSH_eastfall10s,grisSSH_eastfall90s,grisSSH_eastspring00s,
   grisSSH_eastspring10s,grisSSH_eastspring90s,grisSSH_eastsummer00s,grisSSH_eastsummer10s,
   grisSSH_eastsummer90s,grisSSH_eastwinter10s,grisSST_eastfall00s,grisSST_eastfall10s,
   grisSST_eastfall90s,grisSST_eastspring00s,grisSST_eastspring10s,grisSST_eastspring90s,
   grisSST_eastsummer00s,grisSST_eastsummer10s,grisSST_eastsummer90s,grisSST_eastwinter10s,
   grisEastEV,grisEastML,grisEastNV,grisEastSa,grisEastSSH,grisEastSST)
...
datGrisEast <- cbind(grisEast, scores_GrisEast$sites)
datGrisEast$Decade <- factor(datGrisEast$Decade, level = c("90s","00s","10s"))
datGrisEast$Season <- factor(datGrisEast$Season, level = c("Winter","Spring","Summer","Fall"))

ggplot()+
     geom_point(data = datGrisEast, aes(x = RDA1,
                                y = RDA2,
                                color = Decade,
                                fill  = Decade,
                                shape = Season)) +
      ...
      geom_text(data = as.data.frame(scores(rdaGrisEast)$species),
             aes(x = RDA1/optGrisEast_sum$cont$importance[1,1], 
                 ...
                 label = c("Long","Lat")))+
      stat_ellipse(data = datGrisEast, aes(x = RDA1,
                                y = RDA2,
                                color = Decade),
                size = 1,
                level = 0.90,
