...
rare1 <- do.call(rbind, lapply(unique(mms$datasetName), function(i) {
...
# Plot results for the subdatasets collected by Normandeau
rareplot_2 <- ggplot() +
  geom_line(data = subset(rare1, method == "interpolated"), aes(x = t, y = qD, col = datasetName)) + 
  geom_line(data = subset(rare1, method == "extrapolated"), aes(x = t, y = qD, col = datasetName), lty = 3) + 
  geom_point(data = subset(rare1, method == "observed"), aes(x = t, y = qD, col = datasetName), size = 2) + 
  labs(x = "Number of samples", y = "Species richness") + 
  theme_bw(base_size = 14) +
  theme(
    panel.grid.major = element_blank(),
